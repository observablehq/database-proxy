...
export async function server(config, argv) {
  ...
  function index(req, res) {
    ...
    // Authorized CORS
    const allowedOrigin = development ? developmentOrigin : origin;
    const isOriginAllowed =
      req.headers.origin === allowedOrigin || allowedOrigin === "*";
    if (!isOriginAllowed) throw unauthorized("Invalid CORS origin");
