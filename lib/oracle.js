...
export class OracleSingleton {
    ...
    if (!OracleSingleton.instance) {
      try {
        OracleSingleton.instance = import("oracledb").then((module) => {
          ...
          OracleSingleton.types.set(oracledb.DB_TYPE_BOOLEAN, {
            type: boolean,
          });
          OracleSingleton.types.set(oracledb.DB_TYPE_NUMBER, {type: number});
          OracleSingleton.types.set(oracledb.DB_TYPE_BINARY_FLOAT, {
            type: number,
          });
          OracleSingleton.types.set(oracledb.DB_TYPE_BINARY_DOUBLE, {
            type: number,
          });
          OracleSingleton.types.set(oracledb.DB_TYPE_BINARY_INTEGER, {
            type: number,
          });
          OracleSingleton.types.set(oracledb.DB_TYPE_TIMESTAMP, {
            type: string,
            date: true,
          });
          OracleSingleton.types.set(oracledb.DB_TYPE_DATE, {
            type: string,
            date: true,
          });
          OracleSingleton.types.set(oracledb.DB_TYPE_TIMESTAMP_TZ, {
            type: string,
            date: true,
          });
          OracleSingleton.types.set(oracledb.DB_TYPE_TIMESTAMP_LTZ, {
            type: string,
            date: true,
          });
          OracleSingleton.types.set(oracledb.DB_TYPE_INTERVAL_DS, {
            type: string,
            date: true,
          });
          OracleSingleton.types.set(oracledb.DB_TYPE_INTERVAL_YM, {
            type: string,
            date: true,
          });
          OracleSingleton.types.set(oracledb.DB_TYPE_BLOB, {
            type: object,
            buffer: true,
          });
  ...
  static getTypes() {
    if (OracleSingleton.types.isEmpty())
      throw new Error(
        "OracleSingleton not initialized. Call OracleSingleton.initialize() first."
      );
...
function dataTypeSchema({type}) {
  const types = OracleSingleton.getTypes();
  if (types.has(type)) {
