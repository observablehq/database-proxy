...
class OracleSingleton {
  constructor() {
    throw new Error("Use InitOracleSingleton.getInstance()");
  }
  static getInstance() {
    if (!OracleSingleton.instance) {
      OracleSingleton.instance = new InitOracleSingleton();
    }
...
export function addToPools(key, config, pools, {signal}) {
  // Instantiate a single oracledb driver instance
  OracleSingleton.getInstance();
  let credentials;
  ...
  const pool = oracledb.createPool(credentials);
